...
public class Cronica_NotRel extends Activity {
	...
	protected void damenotas() {
	    new Thread() {
	            public void run() {//corremos hilo
	            	String link = _LINK;
	            	//cuando sea columna el proceso es diferente
	            	if(_ESCOLUMNA==8){
	            		listaNot=procesosjsoup.cronica_columnas(link);//procesamos pagina
	            	}
	            	else{//si no es columna sigue proceso general para todas las paginas cronica
	            		listaNot=procesosjsoup.cronica(link);//procesamos pagina
	            	}
	                runOnUiThread(new Runnable() {//cargamos interface aunque no halla terminado peticiones http
	                	public void run() {//cuando termine de procesar el html
	                		...
	                		if(_ESCOLUMNA==8){
	                			lv.setAdapter(dameAdaptadorconfoto());	                		
	                		}else{
	                		...
	                		lv.setOnItemClickListener(new OnItemClickListener() {
	                			   ...
	                			   public void onItemClick(AdapterView<?> adapter, View view, int position, long arg) {
	                			      Intent intent = new Intent(Cronica_NotRel.this, Cronica_NotComp.class);
